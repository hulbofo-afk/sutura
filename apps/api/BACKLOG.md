...
- `GET /analytics/:testId/responses?page=&limit=&sort=createdAt:asc|desc` — réponses privées paginées avec ownership
- Tri sur `title` ou `createdAt` (asc/desc), search ILIKE sur `title`
- tests e2e ajoutés (pagination collections et réponses analytics + ownership)
- 67/67 tests verts
...
✅ Déjà présent et couvert par le flux e2e dans `ReportsService` :

- identité collection/test ;
- KPIs ;
- visualisations textuelles simples ;
...
- Monétisation future : abonnements/paiements Moneroo, à ne pas implémenter pendant la phase gratuite actuelle.
...
- `infra/docker-compose.yml` — 5 services : `postgres` (interne au réseau Docker), `api` (NestJS), `caddy` (reverse proxy + HTTPS auto), `backup` (pg_dump → R2), `migrate` (one-shot `prisma migrate deploy`, profil opt-in).
...
- `infra/backup/` — image custom `postgres:16-alpine + rclone`, dump quotidien à 03:00 UTC, rotation (7 daily + 4 weekly), restauration explicite via `restore.sh --list|--latest|--file <name> --confirm`.
- `infra/deploy.sh` — `git pull` + build image + backup pré-déploiement + `prisma migrate deploy` + restart + health check HTTPS.
- `infra/baseline-existing-db.sh` — contrôle du schéma legacy, backup R2 obligatoire, baseline explicite puis migration additive pour le VPS historique.
...
**Test local validé (podman-compose)** : build image OK, postgres up, init pousse le schéma + seed, API healthy, login + public test + analytics fonctionnels, validation historique 37/37 tests verts.

### Bugs runtime corrigés pendant le test local

- `prisma.config.ts` n'était pas copié dans l'image runtime → Prisma échouait. Ajout de la ligne `COPY --from=build /app/prisma.config.ts ./prisma.config.ts` dans le Dockerfile.
...
- Prisma 7 n'accepte plus `--skip-generate` sur `db push` (flag supprimé) ; le déploiement utilise désormais les migrations versionnées.
...
État historique du 2026-07-24 : **37 tests** verts (23 unit validators + 2 unit scores + 12 e2e). La suite actuelle compte 67 tests.
