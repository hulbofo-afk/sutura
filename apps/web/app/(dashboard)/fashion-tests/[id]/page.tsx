...
function QuestionForm({ testId, question, done }: { testId: Id<"fashionTests">; question?: EditableQuestion; done: () => void }) {
  ...
  return <form onSubmit={submit} className="space-y-5"><h2 className="display-font text-3xl">{question ? "Modifier la question" : "Nouvelle question"}</h2><TextAreaField label="Question" name="text" required defaultValue={question?.text} /><select value={type} onChange={event => setType(event.target.value as QuestionType)}>{types.map(item => <option key={item.value} value={item.value}>{item.label}</option>)}</select>{options && <TextAreaField label="Options" name="options" required defaultValue={question?.options.join("\n")} />}{numeric && <div className="grid grid-cols-2 gap-4"><TextField label="Minimum" name="min" type="number" required defaultValue={question?.min} /><TextField label="Maximum" name="max" type="number" required defaultValue={question?.max} /></div>}<TextField label="Aide" name="helpText" optional defaultValue={question?.helpText} /><label><input type="checkbox" name="required" defaultChecked={question?.required ?? true} /> Obligatoire</label>{error && <p className="text-error">{error}</p>}<Button className="w-full">{question ? "Enregistrer" : "Ajouter"}</Button></form>;
}
